...
p1<- list()
 m1<-meta(ps)$Veg
for(i in levels(m1)) {
  pshm1 <- prune_samples(sample_data(ps1)$Veg == i, ps1)
  n<- rownames(meta(pshm1))
  df1<-oo1[n,]
  df<- cbind(df1, meta(pshm1))
  p1[[i]]<-ggplot(df, aes(x = MDS1, y = MDS2, color = CST))+ geom_point(size= 1.5)+ labs(title = paste0(i), subtitle = 'First Visit') +  xlim(-1, 1)+ ylim(-.5, .5)
  #print(p1[[i]])
}
p2<- list()
for(i in levels(m1)) {
  pshm2 <- prune_samples(sample_data(ps2)$Mixed == i, ps2)
  n<- rownames(meta(pshm2))
  df3<-oo2[n,]
  df2<-cbind(df3, meta(pshm2))
  p2[[i]]<-ggplot(df2, aes(x = MDS1, y = MDS2, color = CST))+ geom_point(size= 1.5)+ labs(title = paste0(i), subtitle = 'Second Visit') + xlim(-1, 1)+ ylim(-.5, .5)
  #print(p2[[i]])
}
```
...
##### ...................
###Heatmap
```{r clust-diverse_CLRleo, eval= T, message=FALSE, echo=FALSE, warning=FALSE, out.width="20%"}
theme_set(theme_bw(base_size = 10))
library(BBmisc)
library(reshape2)
library("RColorBrewer")
...
#pso1<- microbiome::transform(ps, 'clr')
g<- otu_table(ps)
g1<-BBmisc::normalize(g@.Data, 'standardize', range = c(0,1))
colnames(g1)<- colnames(otu_table(ps)@.Data)
j<- otu_table(g1, taxa_are_rows = T)
otu_table(ps)<- j
p <- list()
for(CST in CSTs) {
pshm <- prune_samples(sample_data(ps)$CST == CST, ps)
