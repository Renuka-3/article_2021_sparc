...
sorted.names <- names(sort(x))
bs$Var1 <- factor(bs$Var1, levels=sorted.names)
#bs$Var2 <- factor(bs$Var2, levels = rev(d$Taxon))
#bs$Var3 = str_wrap(bs$Var2, width =30)
bs$Var1 <- factor(gsub(bs$Var1, pattern = "Propionibacterium", replacement = "Propionibacterium/Cutibacterium"))
...
```{r core_prevalance, echo=FALSE, warning=FALSE, message=FALSE, fig.width=20, fig.height=7, fig.show="hold"}
...
##se <- as.data.frame(gsub(df$Genus, pattern = "Propionibacterium", replacement = "Propionibacterium/Cutibacterium"))po <- cbind(ren,se)
bt <- cbind(po,df)
ct <- as.data.frame(bt)
g <- ggplot(ct, aes(x = ct$gsub, y = prevalence)) +
  ...
  labs(y = "Prevalence (%)", x = '', title = "Core prevalence variation") 
print(g)
