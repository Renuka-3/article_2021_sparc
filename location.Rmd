...
## individual abundance bar graph

```{r bar, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=6, out.width="25%", fig.show="hold"}
theme_set(theme_bw(base_size=25))
sp <- aggregate_taxa(phy, "genus")
core.taxa <- core_members(microbiome::transform(sp, "compositional"), detection = 0.1/100, prevalence=20/100) # FOcus on core taxa
...
df$Genus<- gsub((df$genus), pattern = "Propionibacterium", replacement = "Propionibacterium/Cutibacterium")
df <- df %>% arrange(df, Geographical_location) %>% mutate(Sample=factor(Sample, levels=unique(Sample)))
l <- ggplot(df, aes(x = Sample, fill = Genus, y = Abundance)) + geom_bar(stat = "identity", colour = "black") + scale_y_continuous(label=scales::percent) + labs(x = "Sample", y = "Relative Abundance (%)") + facet_grid(Geographical_location ~ .)
print(l)
...
ta <- as.data.frame(phy@tax_table) #for large color options
ta$OTU <- rownames(ta)
colnames(ta)
#j <- j %>% arrange(j, Geographical_location) %>% mutate(Sample=factor(Sample, levels=unique(Sample)))

#sp <- aggregate_taxa(phy, "genus")

#v <- abundances(transform(phy, "compositional"))
p3 <- plot_composition(sp,
  ...
  labs(x = " ", y = "Relative abundance (%)",
    ...
    #geom_bar(stat = "identity", colour = "black") + facet_grid(Geographical_location ~ .)
print(p3)
```
...
```{r plot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=7, out.width="25%", fig.show="hold"}
theme_set(theme_bw(base_size=25))
transform <- microbiome::transform
j <- transform(phy, "compositional")
sp <- aggregate_taxa(j, "genus")
...
md$test <- gsub(md$test, pattern = "Propionibacterium", replacement = "Propionibacterium/Cutibacterium")
##renukadd <- data.frame(Abundance = md) %>% arrange(Abundance)

##dd <- data.frame(Genus = names(md), prevalence = f) %>% arrange(Abundance)

##renukay <- data.frame(test = names(md), detection = md) %>% arrange(Abundance)
##o$taxa <- factor(o$taxa, levels=sorted.names)
##sorted.names <- o$genus 


#attenmt x <- abundances(transform(o, "compositional"))
#attenmtf <- melt(x)
#attenmtf$c <- gsub(f$Var1, pattern = "Propionibacterium", replacement = "Propionibacterium/Cutibacterium")
#names(x) <- gsub(names(x), pattern = "Propionibacterium", replacement = "Propionibacterium/Cutibacterium")
#attenmtdd <- data.frame(Genus = names(f), prevalence = f) %>% arrange(value)
#attenmt sorted.names <- df$Genus 

I <- ggplot(md, aes(x = "Sample", y = "Abundance")) + geom_bar(stat = "identity") +
           labs(y = "Relative abundance (%)", x = '', title = "Core abundance")
print(I)
...
#Visualisation
re = subset_samples(o, Geographical_location=="Ahmednagar")
r <- plot_composition(re,
                      taxonomic.level = "genus",
                      sample.sort = "sample",
                      x.label = " ") +
  guides(fill = guide_legend(ncol = 1)) + 
  scale_y_continuous(label = scales::percent) + scale_fill_brewer("genus", palette = "#8DD3C7", "#FFFFB3", "#BEBADA", "#FB8072", "#80B1D3", "#FDB462", "#B3DE69", "#FCCDE5", "#D9D9D9", "#BC80BD", "#CCEBC5", "#FFED6F", "#A6CEE3", "#1F78B4", "#B2DF8A", "#33A02C", "#FB9A99", "#E31A1C", "#FDBF6F", "#FF7F00", "#CAB2D6", "#6A3D9A", "#FFFF99", "#B15928") +
  labs(x = "Ahmednagar samples", y = "Relative abundance (%)") 
print(r)

nu = subset_samples(o, Geographical_location=="Pune")
n <- plot_composition(nu,
                      taxonomic.level = "genus",
                      sample.sort = "sample",
                      x.label = " ") +
  guides(fill = guide_legend(ncol = 1)) + 
  scale_y_continuous(label = scales::percent) + 
  labs(x = "Pune samples", y = "Relative abundance (%)")
print(n)

ka = subset_samples(o, Geographical_location=="Nashik")
k <- plot_composition(ka,
                      taxonomic.level = "genus",
                      sample.sort = "sample",
                      x.label = " ") +
  guides(fill = guide_legend(ncol = 1)) + 
  scale_y_continuous(label = scales::percent) + 
  labs(x = "Nashik samples", y = "Relative abundance (%)")
print(k)

library("gridExtra")
grid.arrange(r, n, k , nrow = 1)
```

# 3/10/21 at phylum level 
...
```{r plot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=6}
theme_set(theme_bw(base_size=25))
n <- df %>% filter (Geographical_location=="Nashik")
u <- ggplot(n, aes(x = Sample, fill = Phylum, y = Abundance)) + geom_bar(stat = "identity", colour = "black") + 
