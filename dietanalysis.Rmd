...
#df %>% filter(Diet == "Veg") %>% select(diversity_shannon) %>% mean()
head(A)
t <- meta(phy)
t2 <- bind_cols(t,A)
is.data.frame(t) 
is.data.frame(t2)
sapply(split(t2$diversity_shannon, t2$Diet), mean)
```
...
```{r group_comp, echo=FALSE, message=FALSE, fig.width=6, fig.height=6, fig.show="keep", out.width="50%"}
...
```{r pcoa, echo=FALSE, message=FALSE, fig.width=7, fig.height=5, out.width="50%", fig.show="hold"}
theme_set(theme_bw(base_size=25))
library(microbiome)
