...
df <- bind_cols(df, A)
df$index <- df[[index]]
pv <- kruskal.test(data = df, index ~ factor(diet))$p.value
library(ggbeeswarm)
...
p2 <- ggplot(df, aes(fill = diet, x = index)) + geom_histogram(alpha = 0.5) + labs(x = "Shannon Diversity", y = "Density")
print(p2)
...
for (tax in top.taxa) {
  ...
  p <- ggplot(df, aes(x = diet, y = taxa, color = 'diet')) +
